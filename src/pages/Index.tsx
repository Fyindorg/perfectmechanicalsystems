    ...
    <main>
      ...
      >
        ...
        <div className="relative container mx-auto text-center px-4 py-24">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/products" className="inline-flex items-center gap-2 bg-white text-primary font-bold px-8 py-4 rounded-lg text-base hover:bg-white/90 transition-colors shadow-xl">
              {t("hero.viewProducts")} <ArrowRight size={18} />
            </Link>
            <Link to="/contact" className="inline-flex items-center gap-2 bg-white/15 border border-white/40 text-white font-semibold px-8 py-4 rounded-lg text-base hover:bg-white/25 transition-colors backdrop-blur-sm">
              {t("hero.contactUs")}
            </Link>
          </div>
        </div>
        <div className="absolute bottom-0 left-0 right-0 bg-primary/90 backdrop-blur-sm border-t border-white/20">
          <div className="container mx-auto grid grid-cols-2 md:grid-cols-4 divide-x divide-white/20">
            {[
              { num: "17+", label: t("hero.globalBrands") },
              { num: "80+", label: t("hero.countriesServed") },
              { num: "30+", label: t("hero.yearsExperience") },
              { num: "100%", label: t("hero.certifiedProducts") },
            ].map((s) => (
      ...
      <section className="py-20 bg-background" aria-labelledby="about-heading-home">
        <div className="container mx-auto grid md:grid-cols-2 gap-12 items-center px-4">
          <div>
            <p className="text-accent font-semibold text-sm uppercase tracking-widest mb-3">{t("home.whoWeAre")}</p>
            <h2 id="about-heading-home" className="section-title mb-6">{t("home.aboutTitle")}</h2>
            ...
            <Link to="/about" className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-semibold px-6 py-3 rounded-lg hover:bg-primary-light transition-colors">
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-2xl aspect-[4/3] img-hover-zoom">
            ...
            <div className="absolute bottom-6 left-6 right-6 bg-white/10 backdrop-blur-sm rounded-xl p-5 border border-white/20">
              <div className="text-white font-bold text-sm mb-1">{t("home.commercialReg")}</div>
      ...
      <section className="py-20 bg-muted" aria-labelledby="brands-heading-home">
        <div className="container mx-auto px-4">
          <div className="text-center mb-14">
            ...
            <h2 id="brands-heading-home" className="text-3xl md:text-4xl font-bold text-primary mb-4">{t("brands.title")}</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {brands.map((brand) => (
              <div key={brand.name} className="bg-card rounded-xl px-4 py-5 text-center card-shadow hover:elevated-shadow hover:-translate-y-0.5 transition-all duration-200 border border-border flex flex-col items-center justify-center gap-3 min-h-[110px]">
                <div className="img-hover-zoom rounded">
                  <img src={brand.logo} alt={`${brand.name} logo`} className="h-10 w-auto max-w-[100px] object-contain mix-blend-multiply" />
                </div>
      ...
      <section className="py-20 gradient-primary text-primary-foreground" aria-labelledby="products-cta-heading">
        <div className="container mx-auto text-center px-4">
          <h2 id="products-cta-heading" className="text-3xl md:text-4xl font-bold mb-5 text-white">{t("cta.title")}</h2>
          <p className="text-white/80 max-w-2xl mx-auto mb-10 text-lg">{t("cta.desc")}</p>
          <Link to="/products" className="inline-flex items-center gap-2 bg-white text-primary font-bold px-10 py-4 rounded-lg text-lg hover:bg-white/90 transition-colors shadow-xl">
            {t("cta.browse")} <ArrowRight size={20} />
          </Link>
        </div>
      </section>
